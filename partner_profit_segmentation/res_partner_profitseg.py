...
class res_partner_profitseg_segment(models.Model):
    ...
    _sql_constraints = [
        ('profit_limit_consistency', 'CHECK(lower_limit < upper_limit)',  'The lower limit must be lower than the upper one!'),
    ...
    def _check_overlap(self, vals, previous_upper=None, previous_lower=None):
        ...
        if vals.get('lower_limit', False) and vals.get('upper_limit', False):
            if self.search(['&', '|', '&', ('upper_limit', '>', vals.get('lower_limit')), ('lower_limit', '<=', vals.get('upper_limit')), '&', ('lower_limit', '<=', vals.get('upper_limit')), ('upper_limit', '>=', vals.get('lower_limit')), ('id', '!=', self.id)]):
...
class res_partner(models.Model):
    ...
    def get_profit_segment(self):
        ...
        total_turnover = 0.0
        total_cost = 0.0
        for inv_line in self.env['account.invoice.line'].search([('partner_id', '=', self.id),
                ('invoice_id.date_invoice', '>=', time.strftime('%Y-01-01')),('product_id', '!=', False)]):
        ...
        segments = self.env['res.partner.profitseg.segment'].search([('lower_limit', '<=', profit),('upper_limit', '>', profit),])
        if segments:
