...
class hr_holidays(models.Model):
    ...
    def onchange_holiday_employee(self):
        for holiday in self:
            ...
            if holiday.employee_id:
                ...
                user = self.env.user
                working_hours = None
                if holiday.employee_id.calendar_id:
                    working_hours = holiday.employee_id.calendar_id
                elif holiday.employee_id.contract_id and holiday.employee_id.contract_id.working_hours:
                    working_hours = holiday.employee_id.contract_id.working_hours
                if working_hours:
                    ...
                    if from_dt and to_dt:
                        work_hours = working_hours.get_working_hours(start_dt=from_dt,
                                              end_dt=to_dt,
                                              compute_leaves=True,
    ...
    def onchange_date(self):
        for holiday in self:
            ...
            if self.env.context and self.env.context.get('default_type') == 'add':
                return
            ...
            if not holiday.employee_id and (holiday.date_to or holiday.date_from):
    ...
    def _check_holidays(self):
        if not(self.holiday_type != 'employee' or self.type != 'remove' or not self.employee_id or self.holiday_status_id.limit):
            ...
            if leave_hours['remaining_hours'] < 0 or leave_hours['virtual_remaining_hours'] < 0:
                ...
                raise ValidationError(_('The number of remaining hours is not sufficient for this leave type.\n'
    ...
    def name_get(self):
        ...
        for leave in self:
            ...
            leave.employee_id.name, leave.holiday_status_id.name, leave.number_of_hours_temp)))
        return res
