...
class hr_holidays_status(models.Model):
    ...
    def get_hours(self, employee_id):
        ...
        holiday_ids = self.env['hr.holidays'].search(hours_domain)
        '''
        holiday_ids = self.env['hr.employee'].browse(employee_id).holiday_ids.filtered(lambda x: x.state in ['confirm', 'validate1', 'validate'] and x.holiday_status_id == self)

        for holiday in  holiday_ids:
            if holiday.type == 'add':
                result['virtual_remaining_hours'] += holiday.number_of_hours_temp
                if holiday.state == 'validate':
                    result['max_hours'] += holiday.number_of_hours_temp
                    result['remaining_hours'] += holiday.number_of_hours_temp
    ...
    def _user_left_hours(self):
        ...
        if not employee_id:
            employees = self.env['hr.employee'].search([('user_id', '=', self._uid)], limit=1)
            if employees:
    ...
    def name_get(self):
        ...
        for record in self:
            name = record.name
            if not record.limit:
                name += ('  (%.1f Left)' % (record.max_hours-record.hours_taken))
